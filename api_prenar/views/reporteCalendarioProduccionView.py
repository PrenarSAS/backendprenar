...
class CalendarioProduccionStateDetalleView(APIView):
    def get(self, request):
        try:
            # Filtrar calendarios con type=1 y state=2
            calendarios = Calendario.objects.filter(type=1, state=2).select_related('id_pedido', 'id_producto')
            ...
            return Response(
                {
                    "message": "Calendarios de tipo 1 con estado 2 obtenidos exitosamente.",
            ...
            return Response(
                {
                    "message": "Error al obtener los calendarios de tipo 1 con estado 2.",
                    "error": str(e),
                },
